...
- Eureka instance-id: `서비스명:실제포트` (예: `member-regions-service:51234`) — 기동 로그와 Eureka 대시보드에서 확인
...
# 2. .env 설정 후 Member Regions Service 실행
cd ../member-regions-service && ./gradlew bootRun
```

- `bootRun` 은 프로젝트 루트의 `.env` 를 자동 로드합니다.
- IDE에서 실행할 때는 **Run Configuration** `MemberRegionsServiceApplication` 을 사용하거나, `MemberRegionsServiceApplication.java` 에서 main 실행 시 Working directory 가 프로젝트 루트인지 확인하세요.
- `application-local.yml` 이 `optional:file:.env[.properties]` 를 import 하므로 IDE main 실행 시에도 `.env` 가 로드됩니다.

Eureka Dashboard (`http://localhost:8761`) 에서 `MEMBER-REGIONS-SERVICE` 등록을 확인합니다.

## 필수 환경변수 (local)

| 변수 | 설명 |
|------|------|
| `SPRING_DATASOURCE_URL` | MySQL JDBC URL (`member_regions_db` 스키마) |
...
```
com.sparta.member_regions_service/
├── domain/model/              # 도메인 모델
├── application/
│   ├── service/               # Use Case 구현
│   ├── exception/
